function [be_xcorr_self, be_xcorr_other, xaxis, codes] = Shuff_Xcorr_selfother(rdb, betypes, lags, maxlag, normvals)
...
for sess = 1:s
    ...
    for i = 1:length(betypes)
        for j = 1:length(betypes)
            ...
            if ~isempty(curind_comp_all) & ~isempty(curind_ref)
                dbs = 1;
            end
        
           % s_e_mat(curind_comp_all,:) = s_e_mat(curind_comp_all,:) + rand(length(curind_comp_all),1)-.5 * 300;
...
%             end
            ...
            for k = 1:length(curind_ref)
                for n = 1:2
                    randfactor = 300 * (rand(length(curind_comp{n}),1) - 0.5);
                    
                    if ~isempty(curind_comp{n})
                    tse_startbefore = randfactor + s_e_mat(curind_comp{n},:) - s_e_mat(curind_ref(k),1); %time relative to start, animal A
                   ...
                   %then fill in the the start and end bins with one
                    ...
                    tse_endafter = randfactor + s_e_mat(curind_comp{n},:) - s_e_mat(curind_ref(k),2);
