function fns = FindFiles_ni(globfn, varargin)

% fns = FindFiles_ni(globfn, parameters)
%
...
% version L4.2
%
% MODIFIED BY nei 6/13:
%   -changed first for loop to a struct2cell function
%   -added other "dir" info as columns (will be used to compare dates or sizes of different
%       found files)
%
...
if ~isempty(thisdirfiles)
    fns = [cell(length(thisdirfiles),1) struct2cell(thisdirfiles)'];
    [fns{:,1}] = deal(pwd);
end
